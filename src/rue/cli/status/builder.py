...
from rue.context.runtime import ModuleContext, SuiteContext, TestContext
...
class TestsStatusBuilder:
    ...
    def __init__(self, config: Config) -> None:
        self.config = config
        self._issues_by_test_execution_id: dict[UUID, list[TestReportIssue]] = (
            defaultdict(list)
        )
    ...
    def build(
        ...
        self._issues_by_test_execution_id = defaultdict(list)

        items = TestLoader(suitespec.suite_root).load_tests(
            suitespec
        )
        context = SuiteContext(config=self.config)
        with context:
            ...
            connected_by_test_execution_id, graphs_by_test_execution_id = (
                self._collect_static_dependencies(leaves)
            )
            resources_by_test_execution_id = asyncio.run(
                self._preflight(
                    leaves,
                    connected_by_test_execution_id,
                    graphs_by_test_execution_id,
                )
            )
            ...
            for test in tests:
                module_nodes[test.definition.spec.locator.module_path].append(
                    self._finalize_node(
                        test,
                        resources_by_test_execution_id,
                    )
                )
    ...
    def _collect_static_dependencies(
        ...
        connected_by_test_execution_id: dict[
            UUID, tuple[ResourceSpec, ...]
        ] = {}
        graphs_by_test_execution_id: dict[UUID, ResourceGraph] = {}
        for leaf in leaves:
            try:
                ...
                graphs = default_resource_registry.compile_graphs(
                    ...
                    autouse_keys=frozenset({leaf.test_execution_id}),
                )
                graph = graphs[leaf.test_execution_id]
                graphs_by_test_execution_id[leaf.test_execution_id] = graph
                connected_by_test_execution_id[leaf.test_execution_id] = (
                    graph.resolution_order
                )
            except Exception as error:
                self._add_issue(leaf.test_execution_id, "resolve", str(error))
                connected_by_test_execution_id[leaf.test_execution_id] = ()
        return connected_by_test_execution_id, graphs_by_test_execution_id

    async def _preflight(
        self,
        leaves: list[SingleTest],
        connected_by_test_execution_id: dict[UUID, tuple[ResourceSpec, ...]],
        graphs_by_test_execution_id: dict[UUID, ResourceGraph],
    ) -> dict[UUID, dict[str, tuple[ResourceSpec, ...]]]:
        """Resolve graph dependencies enough to report visible resources."""
        resources_by_test_execution_id: dict[
            UUID, dict[str, tuple[ResourceSpec, ...]]
        ] = {}

        for leaf in leaves:
            test_execution_id = leaf.test_execution_id
            connected = connected_by_test_execution_id[test_execution_id]
            if test_execution_id not in graphs_by_test_execution_id:
                resources_by_test_execution_id[test_execution_id] = {}
                continue

            resolver = DependencyResolver(default_resource_registry)
            graph = graphs_by_test_execution_id[test_execution_id]
            visible_resources = {}
            try:
                module_path = leaf.definition.spec.locator.module_path
                with (
                    ModuleContext(module_path),
                    TestContext(test_execution_id=test_execution_id),
                ):
                ...
                self._add_issue(test_execution_id, "resolve", str(error))
            finally:
        ...
        return resources_by_test_execution_id
    ...
    def _add_issue(
        ...
        """Attach one deduplicated status issue to a leaf test execution."""
        if any(
            issue.phase == phase and issue.message == message
            for issue in self._issues_by_test_execution_id[test_execution_id]
        ):
            return
        self._issues_by_test_execution_id[test_execution_id].append(
    ...
    def _finalize_node(
        ...
        return TestReportNode(
            ...
            history=(),
            issues=tuple(
                self._issues_by_test_execution_id.get(
                    test.test_execution_id,
                    (),
                )
            ),
            resources_by_type=resources_by_test_execution_id.get(
                test.test_execution_id, {}
            ),
            children=children,
