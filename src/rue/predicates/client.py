...
"""HTTP client for calling the Rue remote predicate API."""
...
class PredicateAPISettings(BaseSettings):
    ...
    ----------
    base_url
        Service base URL (from ``RUE_API_BASE_URL``).
    api_key
        Bearer token (from ``RUE_API_KEY``).
    ...
    base_url: HttpUrl = Field(
        validation_alias="RUE_API_BASE_URL", default=HttpUrl("https://api.rue.com/api/v1/")
    )
    api_key: SecretStr = Field(validation_alias="RUE_API_KEY", default=SecretStr(""))
    debugging_mode: bool = Field(default=False, validation_alias="RUE_DEBUGGING_MODE")
    ...
    keepalive_expiry: float = Field(default=30.0, validation_alias="RUE_API_KEEPALIVE_EXPIRY")
    ...
    def model_post_init(self, __context: Any) -> None:
        if self.api_key.get_secret_value() == "":
            warnings.warn(
                "RUE_API_KEY is not set. Premium features will raise an exception if used."
            )
...
class PredicateAPIClient:
    ...
    async def request_predicate(self, request: PredicateAPIRequest) -> PredicateAPIResponse:
        ...
        if s.api_key.get_secret_value() == "":
            err_msg = """
            Rue Semantic Predicates is a premium feature and requires a Rue API key.
            Get an API key from rue.com and set it in the RUE_API_KEY environment variable.
            """
