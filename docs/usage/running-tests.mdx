...
# Persist traces locally with the built-in reporter
...
```bash
rue test --db-path /path/to/custom/rue.db
```
...
- OpenTelemetry trace IDs on test executions (if enabled)
...
<CardGroup cols={2}>
  ...
  <Card title="Rue Concept" icon="book" href="/concepts/rue-tests">
