...
In the usual fresh-process setup, `uv run rue test --otel` already writes JSON traces under `.rue/traces/<run_id>/<execution_id>.json` because the built-in `OtelReporter` is registered by default.
...
If `config.reporters` is empty and you do not pass `reporters=[...]`, `Runner` uses all currently registered reporter instances. In a normal fresh process, that includes the built-in `ConsoleReporter` and `OtelReporter`.
...
**Note:** `ConsoleReporter` and `OtelReporter` are built-in. Create custom reporters for JSON, HTML, databases, or external service integrations.
