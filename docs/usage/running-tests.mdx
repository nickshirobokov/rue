...
With Rue's default reporter set, `uv run rue test --otel` already writes JSON traces under `.rue/traces/<run_id>/<execution_id>.json`.
...
If you do not specify reporters, Rue attaches both `ConsoleReporter` and `OtelReporter` by default.
...
async def main():
    ...
    runner = Runner(reporters=[console, otel, json_reporter, slack], otel_enabled=True)
