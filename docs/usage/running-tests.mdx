...
Enable OpenTelemetry capture to record spans from your SUT and tests:

```bash
# Enable OpenTelemetry capture
...
In the usual fresh-process setup, `uv run rue test --otel` already writes JSON traces under `.rue/traces/<run_id>/<execution_id>.json` because the built-in `OtelReporter` is part of the default reporter set.
...
class JsonFileReporter(Reporter):
    ...
    async def on_trace_collected(self, tracer, execution_id) -> None:
        ...
        if session is not None:
            print(f"Collected OpenTelemetry trace for execution {execution_id}")
```
...
```bash
uv run rue test -k "agent_tool_call" --otel -vv
```
