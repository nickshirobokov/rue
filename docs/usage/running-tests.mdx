...
```bash
# Enable OpenTelemetry (writes to .rue/otel-spans.jsonl)
uv run rue test --otel

# Custom output path
uv run rue test --otel --otel-output my-traces.jsonl
```
...
class Reporter(ABC):
    ...
    async def on_otel_enabled(self, output_path: Path) -> None:
        """Called when OpenTelemetry is enabled to report output location."""
```
...
class JsonFileReporter(Reporter):
    ...
    async def on_otel_enabled(self, output_path: Path) -> None:
        print(f"OpenTelemetry enabled: {output_path}")
```
...
- OpenTelemetry references (if enabled)
...
- `otel_spans` - OpenTelemetry spans linked to executions
