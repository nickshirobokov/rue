...
# Session scope: Shared across the entire test run
...
def test_user_queries(user_id, isolated_db, shared_api_client, ml_model):
    ...
    # ml_model: same instance across entire test suite
...
Rue resolves dependencies automatically, respecting scopes: `config` is created once per session, `http_client` once per suite, and `authenticated_client` fresh for each test case.
...
def test_classifier(temp_directory, api_client, ml_model):
    ...
    # ml_model: shared across entire test run
    pass
```
