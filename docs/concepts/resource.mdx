...
- Scoping lifecycles at test, module, or suite level
- Defining directory-scoped `SUITE` overrides through `conftest.py` or `confrue_*.py`
...
def test_user_queries(user_id, isolated_db, shared_api_client, ml_model):
    ...
    # ml_model: same active provider reused across the suite
...
Available scopes:
- `"test"`: New instance per parametrized test case (default)
- `"module"`: One instance per test file/module
- `"suite"`: One active provider instance reused across the suite

If multiple `SUITE` resources share the same name, Rue keeps them side by side
and selects the nearest ancestor provider relative to the consumer spec.
Without a consumer module path, resolution falls back to the latest registered
definition in the first matching scope.

### Directory-Scoped `SUITE` Resources

`SUITE` resources are now best understood as directory-scoped shared resources,
not as one global instance per name.

Put same-name `SUITE` resources in `conftest.py` or `confrue_*.py` files when
different parts of your test tree need different shared setup:
...
Rules to remember:
- Same-name `SUITE` resources can coexist in different directories.
- Rue picks the nearest ancestor provider for the consumer module path.
- Sibling branches do not see each other's child overrides.
- Different directory-specific providers do not share cached state.
- If a `SUITE` resource is resolved without a consumer module path, Rue falls back to the latest active registration.

<Info>
  Put shared `SUITE` resources in `conftest.py` or `confrue_*.py` files so the directory tree controls which provider is active. See [confrue Files](/concepts/confrue-files).
</Info>
...
```
authenticated_client (test-level)
└── http_client (module-level)
    └── config (suite-level)
```

Rue resolves dependencies automatically, respecting scopes: `config` is created
once per active suite provider, `http_client` once per module, and
`authenticated_client` fresh for each test case.
...
@rue.test.iterate.params("user_id", [123, 456, 789])
def test_simple(user_id):
    assert user_id > 0
...
def test_classifier(temp_directory, api_client, ml_model):
    ...
    # ml_model: shared across the suite for the active provider
    pass
```
