...
- Scoping lifecycles at test, module, or run level
- Defining directory-scoped `RUN` overrides through `conftest.py` or `confrue_*.py`
...
# Run scope: Shared across the run for the active provider
@rue.resource(scope="run")
def ml_model():
...
- `"run"`: One active provider instance reused across the run

If multiple `RUN` resources share the same name, Rue keeps them side by side
...
### Directory-Scoped `RUN` Resources

`RUN` resources are now best understood as directory-scoped shared resources,
not as one global instance per name.

Put same-name `RUN` resources in `conftest.py` or `confrue_*.py` files when
...
Rules to remember:
- Same-name `RUN` resources can coexist in different directories.
...
- If a `RUN` resource is resolved without a consumer module path, Rue falls back to the latest active registration.

<Info>
  Put shared `RUN` resources in `conftest.py` or `confrue_*.py` files so the directory tree controls which provider is active. See [confrue Files](/concepts/confrue-files).
</Info>
...
once per active run provider, `http_client` once per module, and
...
# Run scope: Expensive to create, no state mutations
@rue.resource(scope="run")
def ml_model():
