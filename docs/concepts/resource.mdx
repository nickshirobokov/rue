...
# Session scope: Shared across the run for the selected provider
...
def test_user_queries(user_id, isolated_db, shared_api_client, ml_model):
    ...
    # ml_model: same selected provider reused across the run
...
- `"session"`: One selected provider instance reused across the run
...
Rue resolves dependencies automatically, respecting scopes: `config` is created once per selected session provider, `http_client` once per suite, and `authenticated_client` fresh for each test case.
...
def test_classifier(temp_directory, api_client, ml_model):
    ...
    # ml_model: shared across the run for the selected provider
    pass
```
