...
- Defining directory-scoped `SESSION` overrides through `conftest.py` or `confrue_*.py`
...
Put same-name `SESSION` resources in `conftest.py` or `confrue_*.py` files when different parts of your test tree need different shared setup:
...
<Info>
  Put shared `SESSION` resources in `conftest.py` or `confrue_*.py` files so the directory tree controls which provider is active. See [confrue Files](/concepts/confrue-files).
</Info>
