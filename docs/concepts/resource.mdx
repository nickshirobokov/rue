...
- Scoping lifecycles at test, module, or process level
- Defining directory-scoped `PROCESS` overrides through `conftest.py` or `confrue_*.py`
...
# Process scope: Shared across the run for the active provider
...
- `"process"`: One active provider instance reused across the run

If multiple `PROCESS` resources share the same name, Rue keeps them side by side and selects the nearest ancestor provider relative to the requesting test module. Without an active test context, resolution falls back to the flat/latest registered definition.

### Directory-Scoped `PROCESS` Resources

`PROCESS` resources are now best understood as directory-scoped shared resources, not as one global instance per name.

Put same-name `PROCESS` resources in `conftest.py` or `confrue_*.py` files when different parts of your test tree need different shared setup:
...
Rules to remember:
- Same-name `PROCESS` resources can coexist in different directories.
...
- If a `PROCESS` resource is resolved outside an active test context, Rue falls back to the latest active registration.

<Info>
  Put shared `PROCESS` resources in `conftest.py` or `confrue_*.py` files so the directory tree controls which provider is active. See [confrue Files](/concepts/confrue-files).
</Info>
...
└── http_client (module-level)
    └── config (process-level)
```

Rue resolves dependencies automatically, respecting scopes: `config` is created once per active process provider, `http_client` once per module, and `authenticated_client` fresh for each test case.
