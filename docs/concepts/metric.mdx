...
`Metric` is an object that records data points and calculates statistical values. Developers use metrics to track system behavior across suite executions and make data-driven assertions about performance, accuracy, or other measurable properties.
...
- Tracking metrics at different scopes (suite, module, test)
...
def test_classifier_accuracy(my_classifier, input, expected, accuracy: Metric):
...
def test_latency_test(response_time: Metric):
    import time
    start = time.time()
    result = call_api()
...
## Scopes: Suite, Module, Test

Metrics can be scoped to different lifecycle levels. This enables tracking both local statistics (per test execution) and global statistics (across the suite execution).

```python
# Suite scope: collects data across the entire suite execution
...
Available scopes:
- `"suite"`: One metric instance for the entire suite execution (default)
- `"module"`: One instance per test file/module
- `"test"`: New instance for each parametrized test case
...
@rue.test.iterate.params("input", ["good1", "good2", "good3"])
def test_positive_cases(input: str, false_negatives: Metric):
    result = classifier(input)
    with metrics(false_negatives):
        assert result == True

@rue.test.iterate.params("input", ["bad1", "bad2", "bad3"])
def test_negative_cases(input: str, false_positives: Metric):
    result = classifier(input)
...
Choose scope based on what you're measuring. Use test-level metrics for per-test-execution statistics and suite-level metrics for aggregate analysis.

```python
# Test scope: Track each test execution individually
@rue.resource.metric(scope="test")
def case_latency():
    metric = Metric()
    yield metric
...
# Suite scope: Track overall system performance
@rue.resource.metric(scope="suite")
def overall_latency():
    metric = Metric()
    yield metric
