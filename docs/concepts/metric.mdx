...
`Metric` is an object that records data points and calculates statistical values. Developers use metrics to track system behavior across multiple test runs and make data-driven assertions about performance, accuracy, or other measurable properties.
...
Metrics can be scoped to different lifecycle levels. This enables tracking both local statistics (per test case) and global statistics (across all tests).

```python
# Session scope: collects data across the entire test run
...
# Case scope: creates a new metric instance for each test case
...
Available scopes:
- `"process"`: One metric instance for the entire test run (default)
...
Choose scope based on what you're measuring. Use case-level metrics for per-test statistics and session-level metrics for aggregate analysis.

```python
# Case scope: Track each test case individually
@rue.resource.metric(scope="test")
def case_latency():
    metric = Metric()
    yield metric
