...
| `input_kwargs` | `dict[str, Any]` | Read-only plain-dict view of `inputs`, used for `**...` SUT calls |

`sut_input_values` was renamed to `inputs`. When `inputs` is typed, use `case.input_kwargs` when expanding kwargs into a callable.
...
assert case.input_kwargs == {"text": "Sample input"}
```
