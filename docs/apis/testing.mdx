...
@rue.sut
def agent():
    return SUT(Agent(tools="all"), method="run")
...
@rue.sut
def search_agent():
    return SUT(retrieve, validate_cases=cases)

def test_test(agent, rag):
    # Injected values are SUT objects and are called directly
    context = retrieve("Privacy policy")
    output = agent("How do you store my data?", context=context)
```
...
runner = Runner(
    config=RueConfig(otel=True, db_path=".rue/rue.db"),
...
Validate case inputs by passing them to `SUT(..., validate_cases=...)`. Validation runs when the `SUT` object is created and raises if any case does not match the resolved callable or method signature.
...
def my_agent():
    ...
    return SUT(run, validate_cases=cases)
```
