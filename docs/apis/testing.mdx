...
| `Scope.SESSION` | Shared across the run for the selected provider |
...
def expensive_model():
    return load_model()  # Loaded once for the selected provider
...
- If `config.reporters` contains names, `Runner` resolves those names from `Reporter.REGISTRY` and ignores any `reporters=[...]` instances you pass in.
- If `config.reporters` is empty and `reporters=[...]` is provided, `Runner` uses those instances.
- If both are omitted, `Runner` uses all currently registered reporter instances.
- `resource_registry` lets you replace the default global resource registry.
...
OpenTelemetry persistence is configured through reporters. In a normal fresh process, the registered reporter set includes `OtelReporter`; if you supply your own reporters, keep it attached for local `.rue/traces` storage or implement `Reporter.on_trace_collected()` in a custom reporter.
