...
OpenTelemetry persistence is configured through reporters. The default `Runner()` reporter set already includes `OtelReporter`; if you supply your own reporters, keep it attached for local `.rue/traces` storage or implement `Reporter.on_trace_collected()` in a custom reporter.
