...
| `scope` | `Scope \| str` | `Scope.SUITE` | Lifecycle scope: `"test"`, `"module"`, or `"suite"` |
...
@rue.resource.metric(scope="module")
def latency_ms():
...
| `metadata` | `MetricMetadata` | Lifecycle metadata (identity, consumers, timestamps) |
...
@rue.test.iterate.params("input,expected", test_cases)
...
Metadata tracking metric lifecycle, provider identity, and injected consumers.
...
| `identity` | `ResourceSpec` | Metric provider identity, including locator and scope |
| `consumers` | `list[Spec]` | Test/resource specs that consumed the metric through injection |
| `direct_providers` | `list[ResourceSpec]` | Resource providers directly used by this metric provider |
...
def my_metric():
    ...
    print(f"Provider: {meta.identity.locator.function_name}")
    print(f"Scope: {meta.identity.scope}")
    print(f"Consumers: {[spec.locator.function_name for spec in meta.consumers]}")
    print(f"Providers: {[spec.locator.function_name for spec in meta.direct_providers]}")
...
**Note:** `MetricResult` instances are automatically collected and included in suite execution reports.
...
@rue.test.iterate.params("question,context", test_cases)
...
@rue.test.iterate.params("text", test_texts)
...
@rue.test.iterate(50, min_passes=40)
