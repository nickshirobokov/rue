...
| `scope` | `Scope` | Lifecycle scope (PROCESS, MODULE, TEST) |
| `collected_from_tests` | `set[str]` | Names of test functions that contributed |
| `collected_from_resources` | `set[str]` | Names of resources that contributed |
| `collected_from_cases` | `set[str]` | Case IDs that contributed, falling back to case labels when IDs are unavailable |
...
def my_metric():
    ...
    print(f"Contributing tests: {meta.collected_from_tests}")
