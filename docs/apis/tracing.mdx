...
| `trace_content` | `bool \| None` | `None` | Content capture toggle. **Note:** today Rue uses `RUE_TRACE_CONTENT` to control SUT and predicate content attributes; `trace_content` is **not currently applied** to OpenAI/Anthropic client instrumentation. |
...
- `RUE_TRACE_CONTENT`: Set to `"false"` to avoid recording SUT and predicate
  content attributes (`sut.input.*`, `sut.output`, `predicate.input.*`,
  `predicate.message`). This does **not** currently guarantee LLM client
  instrumentation content is redacted.
...
Predicate spans from `@predicate` are available via `get_child_spans()` and
filtering on `rue.predicate` or `rue.predicate.name`.
...
def test_agent_workflow(my_agent, trace_context):
    ...
    # If the test evaluates any @predicate functions
    predicate_spans = [
        span
...
# - Predicate inputs/details (`predicate.input.*`, `predicate.message`)
#
...
- `rue.predicate` / `rue.predicate.name`
- `sut.input.args` / `sut.input.kwargs` / `sut.input.count`
- `sut.output` / `sut.output.type`
- `predicate.value` / `predicate.strict` / `predicate.confidence`
- `predicate.input.actual` / `predicate.input.reference` / `predicate.message`
